...
describe("霧眉教學學員作業範例", () => {
  ...
  it("辨識任務 1 的 A 客群、④ 痛點、無經驗與經歷型定位", () => {
    ...
    expect(optionLabels("stage-01-1-answer-5")).toEqual(["④ 我不知道從哪裡開始"]);
    expect(optionLabels("stage-01-1-answer-14")).toEqual(["沒有。 他第一次認真找這類服務，大概還在自己想辦法"]);
    expect(optionLabels("stage-01-1-answer-15")).toEqual(["A 經歷型：我也走過同一條路"]);
  });
  ...
  it("辨識任務 3 的表格是非答案與任務 4 的女性受眾選項", () => {
    ...
    const gender = task4.fields.find((field) => field.key === "stage-01-4-answer-6");
