  ...
  useEffect(() => {
    ...
    const applyLatest = async () => {
      const [{ data, error }, { data: settingsData, error: settingsError }, { data: targetsData, error: targetsError }] = await Promise.all([
        supabase.from("question_overrides").select("stage_key,task_key,field_key,prompt,description,options,field_type,multiple,sort_order,is_deleted,is_custom,table_key,table_title,table_row,table_column").in("stage_key", ["stage-01", "stage-02"]),
