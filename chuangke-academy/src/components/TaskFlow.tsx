  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [{ data: answerRows }, { data: submissionRows }] = await Promise.all([
        supabase.from("answers").select("stage_key,question_key,value"),
        supabase.from("submissions").select("stage_key,task_key,status,answer_json"),
      ]);
