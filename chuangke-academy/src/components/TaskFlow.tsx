...
type Props = { stage: Stage; courseKey: string; tasks: TaskWithFields[]; workspaceId?: string; nextStageKey?: string };
...
export default function TaskFlow({ stage, courseKey, tasks, workspaceId, nextStageKey }: Props) {
  ...
  const totalAnswered = tasks.reduce((sum, item) => sum + item.fields.filter((field) => !field.hiddenInGroup && field.required !== false && fieldIsActive(field, answers) && fieldIsComplete(field, answers[field.key])).length, 0);
  const totalFields = tasks.reduce((sum, item) => sum + item.fields.filter((field) => !field.hiddenInGroup && field.required !== false && fieldIsActive(field, answers)).length, 0);
  ...
  async function submitTask() {
    ...
    if (!userId) { setCompleted((current) => current.includes(task.key) ? current : [...current, task.key]); goNext(); return; }
