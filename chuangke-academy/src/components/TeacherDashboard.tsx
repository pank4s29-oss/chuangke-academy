...
type Submission = { id: string; user_id: string; stage_key: string; task_key: string; status: "draft" | "completed"; review_status: "pending" | "approved" | "needs_revision"; answer_json: Record<string, unknown>; teacher_feedback: string | null; submitted_at: string | null; updated_at: string };
type Profile = { id: string; display_name: string | null; role: string };
type Props = { optionLabels: Record<string, string> };
const reviewLabels = { pending: "待批改", approved: "已通過", needs_revision: "需要修改" } as const;

export default function TeacherDashboard({ optionLabels }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    const [{ data: submissionRows, error }, { data: profileRows }] = await Promise.all([
      supabase.from("submissions").select("id,user_id,stage_key,task_key,status,review_status,answer_json,teacher_feedback,submitted_at,updated_at").order("updated_at", { ascending: false }),
  ...
  async function grade(reviewStatus: Submission["review_status"]) {
    if (!selected) return;
    const { error } = await supabase.from("submissions").update({ review_status: reviewStatus, teacher_feedback: feedback.trim() || null, graded_at: new Date().toISOString() }).eq("id", selected.id);
    if (error) { setStatus(`批改失敗：${error.message}`); return; }
    setSubmissions((current) => current.map((item) => item.id === selected.id ? { ...item, review_status: reviewStatus, teacher_feedback: feedback.trim() || null } : item));
    setSelected((current) => current ? { ...current, review_status: reviewStatus, teacher_feedback: feedback.trim() || null } : current);
    setStatus("批改結果已保存");
  }
