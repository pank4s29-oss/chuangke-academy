...
function fieldLabel(field: TaskWithFields["fields"][number] | undefined, fallback: string) {
  if (!field) return "未對應題目（請檢查匯入格式）";
  return field.group ? `${field.group}｜${field.prompt}` : field.prompt;
}

function answerText(field: TaskWithFields["fields"][number] | undefined, value: unknown, optionLabels: Record<string, string>) {
  const values = Array.isArray(value) ? value : [value];
  return values.map((item) => {
    const option = field?.options?.find((candidate) => candidate.key === String(item));
    return option?.label ?? optionLabels[String(item)] ?? String(item ?? "");
  }).join("、");
}
...
export default function TeacherDashboard({ optionLabels, stageTitles, stageTasks }: Props) {
  ...
  const selectedFields = selected ? (stageTasks[selected.stage_key] ?? []).flatMap((task) => task.fields) : [];
  const selectedFieldMap = Object.fromEntries(selectedFields.map((field) => [field.key, field]));
  async function saveAnswers() {
    if (!selected) return;
    const { error } = await supabase.from("submissions").update({ answer_json: answerDraft }).eq("id", selected.id);
    if (error) { setStatus(`答案更新失敗：${error.message}`); return; }
    setSubmissions((current) => current.map((item) => item.id === selected.id ? { ...item, answer_json: answerDraft } : item));
    setSelected((current) => current ? { ...current, answer_json: answerDraft } : current);
    setStatus("作答內容已更新");
  }
  ...
  return <main className="min-h-screen bg-[#f5f7f4] text-slate-900"><header className="border-b border-slate-200 bg-white"><div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 lg:px-8"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-teal-700">Teacher workspace</p><h1 className="mt-1 text-2xl font-bold">作業批改後台</h1></div><div className="flex items-center gap-4 text-sm"><span className="text-slate-500">{status}</span><Link href="/teacher/guide" className="font-semibold text-slate-600">教師帳號設定</Link><Link href="/app" className="font-semibold text-teal-700">回到學員端</Link></div></div></header><div className="mx-auto grid max-w-7xl gap-6 px-5 py-8 lg:grid-cols-[330px_1fr] lg:px-8"><aside className="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-slate-200"><div className="flex flex-wrap gap-2">{(["all", "pending", "needs_revision", "approved"] as const).map((key) => <button key={key} onClick={() => setFilter(key)} className={`rounded-full px-3 py-1.5 text-xs font-semibold ${filter === key ? "bg-teal-700 text-white" : "bg-slate-100 text-slate-600"}`}>{key === "all" ? "全部" : reviewLabels[key]}</button>)}</div><div className="mt-5"><label className="block text-xs font-semibold text-slate-500" htmlFor="import-stage">匯入作業所屬階段</label><select id="import-stage" className="mt-2 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm" value={importStage} onChange={(event) => { const nextStage = event.target.value; setImportStage(nextStage); }}>{Object.entries(stageTitles).map(([key, title]) => <option key={key} value={key}>{title}</option>)}</select><p className="mt-2 text-xs leading-5 text-slate-500">此階段包含 {importTasks.length} 個任務，將一次匯入整份作業並建立完整階段藍圖。</p><div className="mt-3"><AssignmentFileImport stageKey={importStage} tasks={importTasks} onImported={(fileName, learnerName, answerCount, taskCount) => { setStatus(`已建立 ${learnerName} 的 ${fileName}，包含 ${taskCount} 個任務、${answerCount} 個答案欄位`); void load(); }} /></div></div><div className="mt-5 space-y-2">{visible.map((item) => <button key={item.id} onClick={() => { setSelected(item); setLearnerName(item.learner_name ?? ""); setAnswerDraft(item.answer_json ?? {}); setFeedback(item.teacher_feedback ?? ""); setAdvice(item.consultant_advice ?? ""); setView("answers"); }} className={`w-full rounded-2xl border p-4 text-left transition ${selected?.id === item.id ? "border-teal-400 bg-teal-50" : "border-slate-200 hover:border-teal-200"}`}><div className="flex items-center justify-between gap-2"><strong className="truncate text-sm">{item.learner_name || (item.user_id ? profiles[item.user_id]?.display_name || `學員 ${item.user_id.slice(0, 6)}` : "未命名學員")}</strong><span className="rounded-full bg-slate-100 px-2 py-1 text-[11px] font-semibold text-slate-600">{reviewLabels[item.review_status]}</span></div><p className="mt-2 text-xs text-slate-500">{item.stage_key} · {item.task_key}</p><p className="mt-1 text-xs text-slate-400">{item.submitted_at ? new Date(item.submitted_at).toLocaleString("zh-TW") : "草稿更新"}</p></button>)}{visible.length === 0 && <p className="py-8 text-center text-sm text-slate-500">目前沒有符合條件的提交。</p>}</div></aside><section className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200 lg:p-8">{selected ? <><div className="flex flex-wrap items-start justify-between gap-4 border-b border-slate-100 pb-5"><div><p className="text-sm font-semibold text-teal-700">{selected.stage_key} · {selected.task_key}</p><div className="mt-2 flex flex-wrap items-center gap-2"><input aria-label="作業名稱" className="rounded-xl border border-slate-200 px-3 py-2 text-xl font-bold outline-none focus:border-teal-500" value={learnerName} onChange={(event) => setLearnerName(event.target.value)} placeholder="輸入學員名稱" /><button onClick={() => void (async () => { const name = learnerName.trim() || "未命名學員"; const { error } = await supabase.from("submissions").update({ learner_name: name }).eq("id", selected.id); if (error) { setStatus(`名稱更新失敗：${error.message}`); return; } setSubmissions((current) => current.map((item) => item.id === selected.id ? { ...item, learner_name: name } : item)); setSelected((current) => current ? { ...current, learner_name: name } : current); setStatus("作業名稱已更新"); })()} className="rounded-xl border border-slate-300 px-3 py-2 text-xs font-semibold text-slate-700">保存名稱</button><button onClick={() => void removeSubmission(selected)} className="rounded-xl bg-rose-50 px-3 py-2 text-xs font-semibold text-rose-700">刪除作業</button></div><p className="mt-2 text-sm text-slate-500">狀態：{selected.status === "completed" ? "學員已提交" : "學員草稿"}</p></div><span className="rounded-full bg-amber-50 px-3 py-1.5 text-sm font-semibold text-amber-800">{reviewLabels[selected.review_status]}</span></div><div className="mt-6 flex flex-wrap gap-2 border-b border-slate-100 pb-3"><button onClick={() => setView("answers")} className={`rounded-xl px-4 py-2 text-sm font-semibold ${view === "answers" ? "bg-slate-900 text-white" : "text-slate-500 hover:bg-slate-100"}`}>本次作答</button><button onClick={() => setView("blueprint")} className={`rounded-xl px-4 py-2 text-sm font-semibold ${view === "blueprint" ? "bg-teal-700 text-white" : "text-slate-500 hover:bg-slate-100"}`}>完整階段藍圖</button></div>{view === "blueprint" ? <div className="mt-6"><StageBlueprint stageTitle={stageTitles[selected.stage_key] ?? selected.stage_key} tasks={stageTasks[selected.stage_key] ?? []} answers={Object.assign({}, ...submissions.filter((item) => item.id === selected.id || (item.stage_key === selected.stage_key && (selected.user_id ? item.user_id === selected.user_id : item.learner_name === selected.learner_name))).map((item) => item.answer_json))} optionLabels={optionLabels} learnerName={learnerName || (selected.user_id ? profiles[selected.user_id]?.display_name || "學員作業" : "未命名學員")} /></div> : <div className="mt-6"><h3 className="text-sm font-bold text-slate-700">作答內容</h3><div className="mt-3 grid gap-3">{Object.entries(selected.answer_json ?? {}).map(([key, value]) => <article key={key} className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><p className="text-sm font-semibold leading-6 text-slate-800">{fieldLabel(selectedFieldMap[key], key)}</p><textarea aria-label={`編輯 ${fieldLabel(selectedFieldMap[key], key)}`} className="mt-2 min-h-20 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-[15px] leading-7 text-slate-800 outline-none focus:border-teal-500" value={answerDraft[key] === undefined ? "" : answerText(selectedFieldMap[key], answerDraft[key], optionLabels)} onChange={(event) => setAnswerDraft((current) => ({ ...current, [key]: event.target.value }))} /></article>)}{Object.keys(selected.answer_json ?? {}).length === 0 && <p className="rounded-2xl bg-slate-50 p-5 text-sm text-slate-500">這份提交沒有可顯示的作答內容。</p>}<button onClick={() => void saveAnswers()} className="mt-4 rounded-xl bg-teal-700 px-4 py-2.5 text-sm font-semibold text-white">保存答案修改</button></div></div>}<div className="mt-7 border-t border-slate-100 pt-6"><label className="block text-sm font-bold text-slate-700" htmlFor="advice">顧問建議</label><textarea id="advice" className="mt-3 min-h-28 w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm leading-7 outline-none focus:border-teal-500 focus:ring-4 focus:ring-teal-100" placeholder="整理給顧問的下一步建議、風險與跟進重點…" value={advice} onChange={(event) => setAdvice(event.target.value)} /><label className="mt-5 block text-sm font-bold text-slate-700" htmlFor="feedback">教師回饋</label><textarea id="feedback" className="mt-3 min-h-32 w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm leading-7 outline-none focus:border-teal-500 focus:ring-4 focus:ring-teal-100" placeholder="寫下具體、可執行的回饋…" value={feedback} onChange={(event) => setFeedback(event.target.value)} /><div className="mt-3 flex flex-wrap gap-2"><button onClick={() => void grade("needs_revision")} className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-2.5 text-sm font-semibold text-amber-800">退回修改</button><button onClick={() => void grade("approved")} className="rounded-xl bg-teal-700 px-4 py-2.5 text-sm font-semibold text-white">批改通過</button></div></div></> : <div className="grid min-h-[520px] place-items-center text-center"><div><div className="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-teal-50 text-2xl text-teal-700">✓</div><h2 className="mt-5 text-xl font-bold">選擇一份作業開始批改</h2><p className="mt-2 max-w-sm text-sm leading-7 text-slate-500">左側會列出學員的草稿與已提交作業，選取後即可檢視答案並留下回饋。</p></div></div>}</section></div></main>;
}
