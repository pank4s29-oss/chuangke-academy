...
export default function QuestionContentEditor({ stageTitles, stageTasks }: Props) {
  ...
  const fields = useMemo(() => stageTasks[stageKey]?.flatMap((task) => task.fields.filter((field) => !field.hiddenInGroup).map((field, index) => ({ task, field, index }))).sort((a, b) => (drafts[a.field.key]?.sort_order ?? a.index) - (drafts[b.field.key]?.sort_order ?? b.index)) ?? [], [stageKey, stageTasks, drafts]);
  ...
  const load = useCallback(async () => {
    setStatus("載入題目覆寫…");
    const { data, error } = await supabase.from("question_overrides").select("stage_key,task_key,field_key,prompt,description,options,field_type,multiple,sort_order").eq("stage_key", stageKey);
    ...
    setRows(loaded); setDrafts(loaded); setStatus(`已載入 ${data?.length ?? 0} 個已修改題目；未修改的題目沿用原始教材。`);
  }, [stageKey, supabase]);
