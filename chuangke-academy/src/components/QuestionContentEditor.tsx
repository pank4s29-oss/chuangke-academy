...
function RecallTargetEditor({ field, stageKey, taskKey, stageTitles, stageTasks, recallRows, saving, onSave }: { field: AssignmentField; stageKey: string; taskKey: string; stageTitles: Record<string, string>; stageTasks: Record<string, TaskWithFields[]>; recallRows: Record<string, RecallTargetOverride>; saving: boolean; onSave: (code: string, value: string) => void }) {
  const code = recallCodeFor(stageKey, taskKey, field.sourceSectionKey);
  if (!code) return null;
  const current = recallRows[code];
  const currentValue = current ? `${current.target_stage_key}|${current.target_task_key}|${current.target_field_key}` : "";
  return <div className="mt-4 rounded-xl border border-teal-200 bg-teal-50/60 p-3"><p className="text-xs font-bold text-teal-900">回顧設定：{code}</p><p className="mt-1 text-xs leading-5 text-teal-800">點擊作業中的這個回顧引用時，開啟下列指定題目的作答。</p><select aria-label={`${code} 回顧目標`} className="mt-2 w-full rounded-lg border border-teal-200 bg-white px-3 py-2 text-sm" value={currentValue} onChange={(event) => onSave(code, event.target.value)} disabled={saving}><option value="">沿用系統自動判斷</option>{Object.entries(stageTasks).flatMap(([targetStage, targetTasks]) => targetTasks.flatMap((targetTask) => targetTask.fields.filter((targetField) => !targetField.hiddenInGroup).map((targetField) => <option key={`${targetStage}|${targetTask.key}|${targetField.key}`} value={`${targetStage}|${targetTask.key}|${targetField.key}`}>{stageTitles[targetStage] ?? targetStage}｜{targetTask.title}｜{fieldLabel(targetField)}</option>)))}</select></div>;
}
